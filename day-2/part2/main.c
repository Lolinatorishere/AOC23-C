...
int Data_Analysis(data *in_memory , int filesize){
    ...
    for(int i = 0 ; i < filesize ; i++){
        ...
        g_min = 0; 
        b_min = 0;
        for(int j = 0 ; j <= in_memory[i].gamesets ; j++){
            if(in_memory[i].red[j] > r_min){
                r_min = in_memory[i].red[j];
            }
            if(in_memory[i].green[j] > g_min){
                g_min = in_memory[i].green[j];
            }
            if(in_memory[i].blue[j] > b_min){
                b_min = in_memory[i].blue[j];
            }
        }
        printf("%i : %i , %i , %i , %i\n" , in_memory[i].gameid , r_min , g_min , b_min , r_min*g_min*b_min);
        total_powers_of += r_min * g_min * b_min;
    }
    printf("total is : %i " , total_powers_of);
    return 0;
}
...
int main(){
    ...
    if(CSV_Input(filename , analysis , filesize) == 1){
        printf("skill issue 1");
        return 1;
    }
    if(Data_Analysis(analysis , filesize) != 0){
        printf("skill issue 2");
        return 1;
    }
    // for(int i = 0 ; i < filesize ; i++){
    //     printf("game %i\n" , analysis[i].gameid);
